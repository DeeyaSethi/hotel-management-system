...
class CustomerInfo:
    def __init__(self, root):
        ...
        # making mainframe to show a message
        top = Frame(self.root)
        top.pack(side="top")
        ...
        #using sql to show the customer details from the database
        def display_info():
            ...
            conn.commit()
            with conn:
                cursor.execute("SELECT Fullname FROM Hotel order by room_number")
                ans = cursor.fetchall()
                for i in ans:
                    self.name_customer_entry.insert(INSERT, i[0] + '\n')

            with conn:
                cursor.execute("SELECT room_number FROM Hotel order by room_number")
                ans = cursor.fetchall()
                for i in ans:
                    self.room_no_customer_entry.insert(INSERT, str(i[0]) + '\n')
        # create button to display all info user asked
...
#mai function taht calls the other functions
