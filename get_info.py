...
#gets unfo of the asked room number
class GetInfo:
    def __init__(self, root):
        ...
        # making a main frame to print mssgs
        top = Frame(self.root)
        top.pack(side="top")
        ...
        def get_info():
            ...
            with conn:
                ...
                if room_number1 in room:
                    with conn:
                        #display all info of asked user
                        cursor.execute("SELECT * FROM Hotel order by room_number")
                        ans = cursor.fetchall()
                        for i in ans:
